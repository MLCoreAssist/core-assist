...
def display_multiple_images(images, labels=None, rows=1, cols=1, rgb_flag=False):
    ...
    n = len(images)
    if rows * cols < n:
        raise ValueError(f"Grid too small! {rows}x{cols} < {n} images. Adjust grid size.")

    # Use multi-threading for fast image loading
    with concurrent.futures.ThreadPoolExecutor() as executor:
        images = list(executor.map(lambda img: load_and_process_image(img, rgb_flag), images))
...
def bbox(img, bboxes, labels=None, confs=None, pad: float = 0.0, text_color=(255, 255, 255), thickness=2, font_scale=0.5 , ret = False ):
    """
    ...
    if labels and len(bboxes) != len(labels):
        raise ValueError("The number of bounding boxes must match the number of labels.")
    if confs and len(bboxes) != len(confs):
        raise ValueError("The number of bounding boxes must match the number of confidence values.")
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if label:
            ...
            cv2.rectangle(bbox_image, (x, y - text_height - baseline), (x + text_width, y), color, -1)

            # Put the text on the image
            cv2.putText(bbox_image, display_text, (x, y - baseline), cv2.FONT_HERSHEY_SIMPLEX, font_scale, text_color, thickness)
...
def segment(img, masks, mask_labels, confs=None, bbox_flag=False, pad_bbox: float = 0.0, 
            text_color=(255, 255, 255), thickness=2, font_scale=1, segment_type="both", ret=False):
    """
    ...
    assert segment_type in ["outline", "filled", "both"], "Invalid segment_type. Choose from 'outline', 'filled', or 'both'."
    ...
    if confs and len(confs) != len(mask_labels):
        raise ValueError("The number of confidence scores must match the number of mask labels.")
    ...
    sorted_confs = [confs[i] if confs else None for i in sorted_indices]  # Sort confs if provided
    ...
    for i, mask in enumerate(sorted_masks):
        ...
        if segment_type in ["filled", "both"]:
            ...
            segmented_img_with_bbox = cv2.addWeighted(segmented_img_with_bbox, 1, color_mask, 0.35, 0)
        ...
        if segment_type in ["outline", "both"]:
            cv2.drawContours(segmented_img_with_bbox, contours, -1, dark_color, thickness)
        ...
        if len(mask_position[0]) > 0:
            ...
            text_y = max(mask_position[0].min() - 10, 10)  # Ensure text stays within bounds
            cv2.putText(segmented_img_with_bbox, mask_label, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX,
                        font_scale, dark_color, thickness)
        ...
        if bbox_flag:
            ...
            calculated_bbox_labels.append(base_label)  # Store bbox label with confidence

    # Process bounding boxes
    if bbox_flag:
        segmented_img_with_bbox = bbox(segmented_img_with_bbox, calculated_bboxes, calculated_bbox_labels, confs=None, 
                                       pad=pad_bbox, text_color=text_color, thickness=thickness, 
                                       font_scale=font_scale, ret=True)
