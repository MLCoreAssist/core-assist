    ...
    def _resolve_dataframe(self):
        ...
        if self._is_csv:
            ...
            for split, group in tqdm(grouped, desc="Processing CSV splits"):
                ...
                anno_path = Path(group["anno_path"].iloc[0])  # Read the first annotation path in the group
    ...
    def _resolve_dataframe(self):
        ...
        if self._is_csv:
            ...
            for split, group in tqdm(grouped, desc="Processing CSV splits"):
                ...
                # Create DataFrame for annotations
                instances = [(x["image_id"], x["category_id"], x["segmentation"]) for x in annots]
                annots_df = pd.DataFrame(instances, columns=["image_id", "class_id", "segmentation"])
                annots_df["category"] = annots_df["class_id"].map(class_map)
                ...
                annots_df[["x_min", "y_min", "width", "height"]] = pd.DataFrame(annots_df["bbox"].tolist(), index=annots_df.index)
                annots_df.drop("bbox", axis=1, inplace=True)
